...
def set_up_preprocessor(mode=0):  # 0 for local, 1 for remote

    with open('config.yml', 'r') as file:
        config0 = yaml.safe_load(file)
    if mode:
        db_name = config0['key']['remote']['db_name']
        client_url = config0['key']['remote']['string']
    else:
        db_name = config0['key']['local']['db_name']
        client_url = config0['key']['local']['string']
    collection_name = 'metadata'

    # Parse the configuration from the YAML file
    with open('coldata/vdb/config.yaml', 'r') as file:
